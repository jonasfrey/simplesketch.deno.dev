...
let f_a_n_u8_o_object = async function(s_id_hashed){
    
        // console.log(o_post_data)
        let a_a_n_u8 = [];
        let n_len_a_n_u8 = 0;
        let a_o_entry = await o_kv.list({prefix: [s_prefix, `o_object`,s_id_hashed]});
        ...
        if(a_n_u8_complete.length == 0){
            let o_v = await o_kv.get([s_prefix, `o_object`,s_id_hashed]);
            if(o_v){
...
let f_handler = async function(o_request){
    ...
    if(o_request.headers.get('Upgrade') == 'websocket'){
        ...
        o_socket.addEventListener("message", async (o_e) => {
            ...
            if(o_websocket_function.s_name == o_websocket_function_set_uuid_hashed.s_name){
                ...
                let a_n_u8_o_object = await f_a_n_u8_o_object(s_uuid_hashed);
                console.log({a_n_u8_o_object})
                if(a_n_u8_o_object.length == 0){
            ...
            if(o_websocket_function.s_name == o_websocket_function_update_o_object.s_name){
                ...
                for(let n_i = 0; n_i < n_arrays; n_i++){
                    ...
                    let v_list = await o_kv.set([s_prefix, `o_object`,s_id_hashed, n_i], a_n_u8_encrypted_slice);
                }
    ...
    if(o_url.pathname == '/read'){
        ...
        let a_a_n_u8 = [];
        let n_len_a_n_u8 = 0;
        let a_o_entry = await o_kv.list({prefix: [s_prefix, `o_object`,o_post_data.s_id_hashed]});
        for await (const o_entry of a_o_entry) {
          console.log(o_entry.key); // ["preferences", "ada"]
        //   console.log(o_entry.value); // { ... }
        //   console.log(o_entry.versionstamp); // "00000000000000010000"
        ...
        if(a_n_u8_complete.length == 0){
            let o_v = await o_kv.get([s_prefix, `o_object`,o_post_data.s_id_hashed]);
            if(o_v){
                a_n_u8_complete = o_v.value;
            }
        }
        
        // let a_n_u8_encrypted = await o_kv.get([s_prefix, `o_object`,o_post_data.s_id_hashed]);
        // console.log(a_n_u8_encrypted);
        return new Response(
            a_n_u8_complete,
    ...
    if(o_url.pathname == '/delete'){
        let o_post_data = await o_request.json();
        // console.log(o_post_data)
        let a_v_key = [s_prefix, `o_object`,o_post_data.s_id_hashed]
